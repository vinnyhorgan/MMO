...
function gooi.draw(group)
  ...
  if compWithTooltip and os ~= "Android" and os ~= "iOS" and gooi.desktop then
    ...
    local xTT = math.floor(utils.mouseX() / gooi.sx)

    if ((utils.mouseX() / gooi.sx) + ttf:getWidth(text)) >= gooi.canvas:getWidth() then
    ...
    love.graphics.rectangle("fill",
      xTT - httf,
      math.floor((utils.mouseY() / gooi.sy) - httf * 1.5),
    ...
    love.graphics.print(text,
      xTT,
      math.floor((utils.mouseY() / gooi.sy) - httf))
  end
...
function gooi.pressed(id, xt, yt)
  local x = xt or utils.mouseX()
  local y = yt or utils.mouseY()
  x = x / gooi.sx
  y = y / gooi.sy
  gooi.focused = nil
  for k, c in pairs(gooi.components) do
